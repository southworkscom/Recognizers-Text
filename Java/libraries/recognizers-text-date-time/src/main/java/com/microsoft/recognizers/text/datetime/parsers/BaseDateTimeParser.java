...
public class BaseDateTimeParser implements  IDateTimeParser {
        ...
        if (er.type.equals(getParserName())) {
            ...
            if (innerResult.getSuccess())
            {
                Map<String, String> futureResolution = ImmutableMap.<String, String>builder()
                    .put(TimeTypeConstants.DATETIME, FormatUtil.formatDateTime((LocalDateTime) innerResult.getFutureValue()))
                    .build();
                innerResult.setFutureResolution(futureResolution);

                Map<String, String> pastResolution = ImmutableMap.<String, String>builder()
                    .put(TimeTypeConstants.DATETIME, FormatUtil.formatDateTime((LocalDateTime) innerResult.getPastValue()))
                    .build();
    ...
    private DateTimeResolutionResult mergeDateAndTime(String text, LocalDateTime reference) {
            ...
            if (ersTime.size() == 1) {
                ...
                if (!numErs.isEmpty() && ersDate.size() == 1) {
                    for (ExtractResult num : numErs) {
                        ...
                        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(config.getDateNumberConnectorRegex(), middleStr)).findFirst();
        ...
        } else if (RegExpUtility.getMatches(config.getAMTimeRegex(), text).length != 0 && hour >= Constants.HalfDayHourCount) {
            hour -= Constants.HalfDayHourCount;
        }
        ...
        if (hour <= Constants.HalfDayHourCount
                && RegExpUtility.getMatches(config.getPMTimeRegex(), text).length == 0
        ...
        result.setFutureValue(DateUtil.safeCreateFromMinValue(
            futureDate.getYear(),
            futureDate.getMonthValue(),
            futureDate.getDayOfMonth(),
            hour, minute, second)
        );
        result.setPastValue(DateUtil.safeCreateFromMinValue(
            pastDate.getYear(),
    ...
    private DateTimeResolutionResult parseTimeOfToday(String text, LocalDateTime reference) {
        ...
        Optional<Match> wholeMatch = Arrays.stream(RegExpUtility.getMatches(config.getSimpleTimeOfTodayAfterRegex(), trimmedText)).findFirst();
        if (!(wholeMatch.isPresent() && wholeMatch.get().length == trimmedText.length())) {
            wholeMatch = Arrays.stream(RegExpUtility.getMatches(config.getSimpleTimeOfTodayBeforeRegex(), trimmedText)).findFirst();
        }
        ...
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(config.getSpecificTimeOfDayRegex(), trimmedText)).findFirst();
        ...
        if (match.isPresent()) {
            ...
            LocalDateTime dateResult = DateUtil.safeCreateFromMinValue(
                date.getYear(),
                date.getMonthValue(),
                date.getDayOfMonth(),
                hour, minute, second
    ...
    private DateTimeResolutionResult parserDurationWithAgoAndLater(String text, LocalDateTime reference) {
        return AgoLaterUtil.parseDurationWithAgoAndLater(text, reference, config.getDurationExtractor(), config.getDurationParser(), config.getUnitMap(), config.getUnitRegex(), config.getUtilityConfiguration(), config::getSwiftDay);
    }
}

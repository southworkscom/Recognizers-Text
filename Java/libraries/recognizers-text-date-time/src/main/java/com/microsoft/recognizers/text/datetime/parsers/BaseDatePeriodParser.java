...
public class BaseDatePeriodParser implements IDateTimeParser {
        ...
        if (er.type.equals(parserName)) {
            ...
            if (innerResult.getSuccess()) {
                if (innerResult.getMod() != null && innerResult.getMod().equals(Constants.BEFORE_MOD)) {
                    innerResult
                            .setFutureResolution(ImmutableMap.<String, String>builder()
                                    .put(TimeTypeConstants.END_DATE,
                                            FormatUtil.formatDate((LocalDateTime) innerResult.getFutureValue()))
                                    .build());

                    innerResult.setPastResolution(ImmutableMap.<String, String>builder().put(TimeTypeConstants.END_DATE,
                            FormatUtil.formatDate((LocalDateTime) innerResult.getPastValue())).build());
                    ...
                    innerResult.setFutureResolution(ImmutableMap.<String, String>builder()
                            .put(TimeTypeConstants.START_DATE, FormatUtil.formatDate(
                                    ((Pair<LocalDateTime, LocalDateTime>) innerResult.getFutureValue()).getValue0()))
                            .put(TimeTypeConstants.END_DATE, FormatUtil.formatDate(
                                    ((Pair<LocalDateTime, LocalDateTime>) innerResult.getFutureValue()).getValue1()))
                            .build());

                    innerResult.setPastResolution(ImmutableMap.<String, String>builder()
                            .put(TimeTypeConstants.START_DATE, FormatUtil.formatDate(
                                    ((Pair<LocalDateTime, LocalDateTime>) innerResult.getPastValue()).getValue0()))
                            .put(TimeTypeConstants.END_DATE, FormatUtil.formatDate(
                                    ((Pair<LocalDateTime, LocalDateTime>) innerResult.getPastValue()).getValue1()))
                            .build());
                } else {
    ...
    // Process case like "from|between START to|and END" where START/END can be
    // dateRange or datePoint
    private DateTimeResolutionResult parseComplexDatePeriod(String text, LocalDateTime referenceDate) {
        ...
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getComplexDatePeriodRegex(), text))
                .findFirst();
        ...
        if (match.isPresent()) {
            ...
            DateTimeResolutionResult startResolution = parseSingleTimePoint(match.get().getGroup("start").value.trim(),
                    referenceDate);
            ...
            if (startResolution.getSuccess()) {
                DateTimeResolutionResult endResolution = parseSingleTimePoint(match.get().getGroup("end").value.trim(),
                        referenceDate);
    ...
    private DateTimeResolutionResult parseBaseDatePeriod(String text, LocalDateTime referenceDate) {
        ...
        // Parse duration should be at the end since it will extract "the last week"
    ...
    private DateTimeResolutionResult parseOrdinalNumberWithCenturySuffix(String text, LocalDateTime referenceDate) {
        ...
        if (er.isPresent() && er.get().start + er.get().length < text.length()) {
            ...
            if (Arrays.stream(RegExpUtility.getMatches(this.config.getCenturySuffixRegex(), afterString)).findFirst()
                    .isPresent()) {
                ParseResult number = this.config.getNumberParser().parse(er.get());

                if (number.value != null) {
                    // Note that 1st century means from year 0 - 100
                    int startYear = (Math.round(((Double) number.value).floatValue()) - 1)
                            * Constants.CenturyYearsCount;
                    LocalDateTime startDate = DateUtil.safeCreateFromMinValue(startYear, 1, 1);
                    LocalDateTime endDate = DateUtil.safeCreateFromMinValue(startYear + Constants.CenturyYearsCount, 1,
                            1);
    ...
    private DateTimeResolutionResult parseDatePointWithAgoAndLater(String text, LocalDateTime referenceDate) {
        ...
        if (er.isPresent()) {
            ...
            boolean isAgo = Arrays.stream(RegExpUtility.getMatches(this.config.getAgoRegex(), er.get().text))
                    .findFirst().isPresent();
            boolean isLater = Arrays.stream(RegExpUtility.getMatches(this.config.getLaterRegex(), er.get().text))
                    .findFirst().isPresent();
            ...
            if (!StringUtility.isNullOrEmpty(beforeString) && (isAgo || isLater)) {
                ...
                if (er.get().text.contains("today") || er.get().text.contains("now")) {
                    Optional<Match> match = Arrays
                            .stream(RegExpUtility.getMatches(this.config.getWithinNextPrefixRegex(), beforeString))
                            .findFirst();
                    if (match.isPresent()) {
                ...
                isLessThanOrWithIn = isLessThanOrWithIn
                        || (Arrays.stream(RegExpUtility.getMatches(this.config.getLessThanRegex(), beforeString))
                                .findFirst().isPresent());
                isMoreThan = Arrays.stream(RegExpUtility.getMatches(this.config.getMoreThanRegex(), beforeString))
                        .findFirst().isPresent();

                DateTimeParseResult pr = this.config.getDateParser().parse(er.get(), referenceDate);
                Optional<ExtractResult> durationExtractionResult = this.config.getDurationExtractor()
                        .extract(er.get().text).stream().findFirst();
                ...
                if (durationExtractionResult.isPresent()) {
                    ...
                    long durationInSeconds = Math
                            .round((Double) ((DateTimeResolutionResult) (duration.value)).getPastValue());
    ...
    private DateTimeResolutionResult parseSingleTimePoint(String text, LocalDateTime referenceDate) {
        ...
        DateTimeResolutionResult ret = new DateTimeResolutionResult();
        ExtractResult er = this.config.getDateExtractor().extract(text, referenceDate).stream().findFirst()
                .orElse(null);

        if (er != null) {
            Optional<Match> match = Arrays
                    .stream(RegExpUtility.getMatches(this.config.getWeekWithWeekDayRangeRegex(), text)).findFirst();
    ...
    private DateTimeResolutionResult parseSimpleCases(String text, LocalDateTime referenceDate) {
        ...
        Optional<Match> match = Arrays
                .stream(RegExpUtility.getMatches(this.config.getMonthFrontBetweenRegex(), trimmedText)).findFirst();
        String beginLuisStr;
        String endLuisStr;
        ...
        if (!match.isPresent()) {
            match = Arrays.stream(RegExpUtility.getMatches(this.config.getMonthFrontSimpleCasesRegex(), trimmedText))
                    .findFirst();
        }
    ...
    private DateTimeResolutionResult parseOneWordPeriod(String text, LocalDateTime referenceDate) {
        ...
        Optional<Match> match = Arrays
                .stream(RegExpUtility.getMatches(this.config.getOneWordPeriodRegex(), trimmedText)).findFirst();

        if (!(match.isPresent() && match.get().index == 0 && match.get().length == trimmedText.length())) {
            match = Arrays.stream(RegExpUtility.getMatches(this.config.getLaterEarlyPeriodRegex(), trimmedText))
                    .findFirst();
        }

        // For cases "that week|month|year"
        if (!(match.isPresent() && match.get().index == 0 && match.get().length == trimmedText.length())) {
            match = Arrays.stream(RegExpUtility.getMatches(this.config.getReferenceDatePeriodRegex(), trimmedText))
                    .findFirst();
            isRef = true;
        ...
        if (match.isPresent() && match.get().index == 0 && match.get().length == trimmedText.length()) {
            ...
            if (this.config.isYearToDate(trimmedText)) {
                ...
                ret.setFutureValue(
                        new Pair<>(DateUtil.safeCreateFromMinValue(referenceDate.getYear(), 1, 1), referenceDate));
                ret.setPastValue(
                        new Pair<>(DateUtil.safeCreateFromMinValue(referenceDate.getYear(), 1, 1), referenceDate));
            ...
            if (!StringUtility.isNullOrEmpty(monthStr)) {
                ...
                if (this.config.isWeekOnly(trimmedText)) {
                    LocalDateTime monday = DateUtil.thisDate(referenceDate, DayOfWeek.MONDAY.getValue())
                            .plusDays(7 * swift);

                    ret.setTimex(isRef ? TimexUtility.generateWeekTimex() : TimexUtility.generateWeekTimex(monday));

                    LocalDateTime beginDate = DateUtil.thisDate(referenceDate, DayOfWeek.MONDAY.getValue())
                            .plusDays(7 * swift);

                    LocalDateTime endValue = DateUtil.thisDate(referenceDate, DayOfWeek.SUNDAY.getValue())
                            .plusDays(7 * swift);
                ...
                if (this.config.isWeekend(trimmedText)) {
                    LocalDateTime beginDate = DateUtil.thisDate(referenceDate, DayOfWeek.SATURDAY.getValue())
                            .plusDays(7 * swift);
                    LocalDateTime endValue = DateUtil.thisDate(referenceDate, DayOfWeek.SUNDAY.getValue())
                            .plusDays(7 * swift);
                    LocalDateTime endDate = inclusiveEndPeriod ? endValue : endValue.plusDays(1);
                    ...
                    ret.setTimex(
                            isRef ? TimexUtility.generateWeekendTimex() : TimexUtility.generateWeekendTimex(beginDate));
        ...
        LocalDateTime futureEnd = inclusiveEndPeriod ? futureStart.plusMonths(1).minusDays(1)
                : futureStart.plusMonths(1);
        ...
        if (earlyPrefix) {
            futureEnd = inclusiveEndPeriod ? DateUtil.safeCreateFromMinValue(futureYear, month, 15)
                    : DateUtil.safeCreateFromMinValue(futureYear, month, 15).plusDays(1);
            pastEnd = inclusiveEndPeriod ? DateUtil.safeCreateFromMinValue(pastYear, month, 15)
            ...
            futureEnd = inclusiveEndPeriod ? futureStart.plusDays(10) : futureStart.plusDays(11);
            pastEnd = inclusiveEndPeriod ? pastStart.plusDays(10) : pastStart.plusDays(11);
        } else if (latePrefix) {
    ...
    private DateTimeResolutionResult parseMonthWithYear(String text, LocalDateTime referenceDate) {
        ...
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getMonthWithYear(), text))
                .findFirst();
        if (!match.isPresent()) {
    ...
    private DateTimeResolutionResult parseYear(String text, LocalDateTime referenceDate) {
        ...
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getYearPeriodRegex(), text))
                .findFirst();
        Optional<Match> matchMonth = Arrays.stream(RegExpUtility.getMatches(this.config.getMonthWithYear(), text))
                .findFirst();
        ;
        ...
        if (match.isPresent() && !matchMonth.isPresent()) {
            ...
            if (beginYear != Constants.InvalidYear && endYear != Constants.InvalidYear) {
                ...
                ret.setTimex(String.format("(%s,%s,P%sY)", FormatUtil.luisDate(beginDay), FormatUtil.luisDate(endDay),
                        (endYear - beginYear)));
    ...
    private DateTimeResolutionResult mergeTwoTimePoints(String text, LocalDateTime referenceDate) {
        ...
        ret.setTimex(String.format("(%s,%s,P%sD)", ((DateTimeParseResult) pr1).timexStr,
                ((DateTimeParseResult) pr2).timexStr, diffDays));
        ret.setFutureValue(new Pair<>(futureBegin, futureEnd));
        ret.setPastValue(new Pair<>(pastBegin, pastEnd));
        ret.setSuccess(true);
    ...
    private DateTimeResolutionResult parseDuration(String text, LocalDateTime referenceDate) {
        ...
        if (ers.size() == 1) {
            ...
            String afterStr = text
                    .substring(((pr.start != null) ? pr.start : 0) + ((pr.length != null) ? pr.length : 0)).trim()
                    .toLowerCase();
            ...
            if (pr.value != null) {
                ...
                Optional<Match> prefixMatch = Arrays.stream(RegExpUtility.getMatches(config.getPastRegex(), beforeStr))
                        .findFirst();
                if (prefixMatch.isPresent()) {
                    getModAndDateResult = getModAndDate(beginDate, endDate, referenceDate, durationResult.getTimex(),
                            false);
                    beginDate = getModAndDateResult.beginDate;

                } else {
                    Optional<Match> suffixMatch = Arrays
                            .stream(RegExpUtility.getMatches(config.getPastRegex(), afterStr)).findFirst();
                    if (suffixMatch.isPresent()) {
                        getModAndDateResult = getModAndDate(beginDate, endDate, referenceDate,
                                durationResult.getTimex(), false);
                        beginDate = getModAndDateResult.beginDate;
                    }
                }

                // Handle the "within two weeks" case which means from today to the end of next
                // two weeks
                // Cases like "within 3 days before/after today" is not handled here (4th
                // condition)
                prefixMatch = Arrays.stream(RegExpUtility.getMatches(config.getWithinNextPrefixRegex(), beforeStr))
                        .findFirst();
                if (prefixMatch.isPresent() && prefixMatch.get().length == beforeStr.length()
                        && DurationParsingUtil.isDateDuration(durationResult.getTimex())
                        && StringUtility.isNullOrEmpty(afterStr)) {
                    getModAndDateResult = getModAndDate(beginDate, endDate, referenceDate, durationResult.getTimex(),
                            true);
                    beginDate = getModAndDateResult.beginDate;
                    endDate = getModAndDateResult.endDate;

                    // In GetModAndDate, this "future" resolution will add one day to
                    // beginDate/endDate, but for the "within" case it should start from the current
                    // day.
                ...
                if (prefixMatch.isPresent() && prefixMatch.get().length == beforeStr.length()) {
                    ...
                    Optional<Match> suffixMatch = Arrays
                            .stream(RegExpUtility.getMatches(config.getFutureRegex(), afterStr)).findFirst();
                    if (suffixMatch.isPresent()) {
                        getModAndDateResult = getModAndDate(beginDate, endDate, referenceDate,
                                durationResult.getTimex(), true);
                        beginDate = getModAndDateResult.beginDate;
                        endDate = getModAndDateResult.endDate;
                    }
                }

                Optional<Match> futureSuffixMatch = Arrays
                        .stream(RegExpUtility.getMatches(config.getFutureSuffixRegex(), afterStr)).findFirst();
                ...
                prefixMatch = Arrays.stream(RegExpUtility.getMatches(config.getInConnectorRegex(), beforeStr))
                        .findFirst();
                if (prefixMatch.isPresent() && prefixMatch.get().length == beforeStr.length()
        ...
        // Parse "rest of"
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getRestOfDateRegex(), text))
                .findFirst();
        if (match.isPresent()) {
        ...
        if (!beginDate.equals(endDate) || restNowSunday) {
            ...
            ret.setTimex(
                    String.format("(%s,%s,%s)", FormatUtil.luisDate(beginDate), FormatUtil.luisDate(endDate), timex));
            ret.setFutureValue(new Pair<>(beginDate, endDate));
            ret.setPastValue(new Pair<>(beginDate, endDate));
            ret.setSuccess(true);
    ...
    private GetModAndDateResult getModAndDate(LocalDateTime beginDate, LocalDateTime endDate,
            LocalDateTime referenceDate, String timex, boolean future) {
        if (future) {
    ...
    private DateTimeResolutionResult parseWeekOfMonth(String text, LocalDateTime referenceDate) {
        ...
        String trimmedText = text.trim().toLowerCase();
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getWeekOfMonthRegex(), trimmedText))
                .findFirst();
        if (!(match.isPresent() && match.get().length == text.length())) {
            return ret;
        }
    ...
    private DateTimeResolutionResult parseWeekOfYear(String text, LocalDateTime referenceDate) {
        DateTimeResolutionResult ret = new DateTimeResolutionResult();
        String trimmedText = text.trim().toLowerCase();
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getWeekOfYearRegex(), trimmedText))
                .findFirst();
        if (!(match.isPresent() && match.get().length == text.length())) {
            return ret;
        }
        ...
        if (this.config.isLastCardinal(cardinalStr)) {
            ...
            weekNum = LocalDate.of(lastDay.getYear(), lastDay.getMonthValue(), lastDay.getDayOfMonth())
            ...
            weekNum = LocalDate
                    .of(targetWeekMonday.getYear(), targetWeekMonday.getMonthValue(), targetWeekMonday.getDayOfMonth())
                    .get(IsoFields.WEEK_OF_WEEK_BASED_YEAR);
            ...
            weekNum = LocalDate.of(firstDay.getYear(), firstDay.getMonthValue(), firstDay.getDayOfMonth())
                    .get(IsoFields.WEEK_OF_WEEK_BASED_YEAR);
            if (weekNum != 1) {
        ...
        ret.setFutureValue(inclusiveEndPeriod ? new Pair<>(targetWeekMonday, targetWeekMonday.plusDays(6))
                : new Pair<>(targetWeekMonday, targetWeekMonday.plusDays(7)));

        ret.setPastValue(inclusiveEndPeriod ? new Pair<>(targetWeekMonday, targetWeekMonday.plusDays(6))
                : new Pair<>(targetWeekMonday, targetWeekMonday.plusDays(7)));
    ...
    private DateTimeResolutionResult parseHalfYear(String text, LocalDateTime referenceDate) {
        DateTimeResolutionResult ret = new DateTimeResolutionResult();
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getAllHalfYearRegex(), text))
                .findFirst();
        ...
        LocalDateTime beginDate = DateUtil.safeCreateFromMinValue(year,
                (halfNum - 1) * Constants.SemesterMonthCount + 1, 1);
        LocalDateTime endDate = DateUtil.safeCreateFromMinValue(year, halfNum * Constants.SemesterMonthCount, 1)
                .plusMonths(1);
        ret.setFutureValue(new Pair<>(beginDate, endDate));
        ret.setPastValue(new Pair<>(beginDate, endDate));
    ...
    private DateTimeResolutionResult parseQuarter(String text, LocalDateTime referenceDate) {
        DateTimeResolutionResult ret = new DateTimeResolutionResult();
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getQuarterRegex(), text))
                .findFirst();
        ...
        LocalDateTime beginDate = DateUtil.safeCreateFromMinValue(year,
                (quarterNum - 1) * Constants.TrimesterMonthCount + 1, 1);
        LocalDateTime endDate = DateUtil.safeCreateFromMinValue(year, quarterNum * Constants.TrimesterMonthCount, 1)
                .plusMonths(1);
        ...
        if (noSpecificYear) {
            if (endDate.compareTo(referenceDate) < 0) {
                ...
                LocalDateTime futureBeginDate = DateUtil.safeCreateFromMinValue(year + 1,
                        (quarterNum - 1) * Constants.TrimesterMonthCount + 1, 1);
                LocalDateTime futureEndDate = DateUtil
                        .safeCreateFromMinValue(year + 1, quarterNum * Constants.TrimesterMonthCount, 1).plusMonths(1);
                ret.setFutureValue(new Pair<>(futureBeginDate, futureEndDate));
            } else if (endDate.compareTo(referenceDate) > 0) {
                ret.setFutureValue(new Pair<>(beginDate, endDate));

                LocalDateTime pastBeginDate = DateUtil.safeCreateFromMinValue(year - 1,
                        (quarterNum - 1) * Constants.TrimesterMonthCount + 1, 1);
                LocalDateTime pastEndDate = DateUtil
                        .safeCreateFromMinValue(year - 1, quarterNum * Constants.TrimesterMonthCount, 1).plusMonths(1);
    ...
    private DateTimeResolutionResult parseWeekOfDate(String text, LocalDateTime referenceDate) {
        ...
        if (match.isPresent() && ex.size() == 1) {
            DateTimeResolutionResult pr = (DateTimeResolutionResult) config.getDateParser().parse(ex.get(0),
                    referenceDate).value;
            if (config.getOptions().match(DateTimeOptions.CalendarMode)) {
                LocalDateTime monday = DateUtil.thisDate((LocalDateTime) pr.getFutureValue(),
    ...
    private DateTimeResolutionResult parseWhichWeek(String text, LocalDateTime referenceDate) {
        DateTimeResolutionResult ret = new DateTimeResolutionResult();
        Optional<Match> match = Arrays.stream(RegExpUtility.getMatches(this.config.getWhichWeekRegex(), text))
                .findFirst();
        if (match.isPresent() && match.get().length == text.length()) {
    ...
    private static DateTimeResolutionResult getWeekOfMonth(int cardinal, int month, int year,
            LocalDateTime referenceDate, boolean noYear) {
        DateTimeResolutionResult ret = new DateTimeResolutionResult();
        ...
        ret.setFutureValue(inclusiveEndPeriod ? new Pair<>(futureDate, futureDate.plusDays(6))
                : new Pair<>(futureDate, futureDate.plusDays(7)));
        ret.setPastValue(inclusiveEndPeriod ? new Pair<>(pastDate, pastDate.plusDays(6))
    ...
    private DateTimeResolutionResult parseDecade(String text, LocalDateTime referenceDate) {
        ...
        String trimmedText = text.trim();
        Optional<Match> match = Arrays
                .stream(RegExpUtility.getMatches(this.config.getDecadeWithCenturyRegex(), trimmedText)).findFirst();
        String beginLuisStr;
        String endLuisStr;
        ...
        if (match.isPresent() && match.get().index == 0 && match.get().length == trimmedText.length()) {
            ...
            if (!StringUtility.isNullOrEmpty(centuryStr)) {
                if (!IntegerUtility.canParse(centuryStr)) {
                    if (this.config.getNumbers().containsKey(centuryStr)) {
                        ...
                        firstTwoNumOfYear = Math
                                .round(((Double) (this.config.getNumberParser().parse(er.get(0)).value != null
            ...
            match = Arrays.stream(RegExpUtility.getMatches(this.config.getRelativeDecadeRegex(), trimmedText))
                    .findFirst();
            if (match.isPresent() && match.get().index == 0 && match.get().length == trimmedText.length()) {
                inputCentury = true;
